...
namespace CommandParseEr{
        ...
        // 命令类型枚举
        enum class CommandType { START_TASK, STOP_TASK, CHECK_CHECKBOX, UNCHECK_CHECKBOX, UNKNOWN,WOW_SAY ,LIFT_CLICK,RIGHT_CLICK,ACTIVE_PROCESS};
       ...
       inline CommandType parseCommand(const std::wstring& command) {
            ...
            if (command == L"ACTIVE_PROCESS") return CommandType::ACTIVE_PROCESS;
       ...
       inline int test() {
            ...
            //选中选择任务框0
			TYPE:2|COMMAND:ACTIVE_PROCESS|CHECKBOX_ID:999 //-1表示激活所有窗口
