...
bool is_all_digits(const std::wstring& s) {
    auto it = std::find(s.begin(), s.end(), L'\0');
    return it != s.begin() && std::all_of(s.begin(), it, iswdigit);
}
...
//遍历获取所有ListBoxItemData
std::vector<ListBoxItemData*> GetAllListBoxItemData(HWND hListBox) {
    std::vector<ListBoxItemData*> items;
    int count = SendMessageW(hListBox, LB_GETCOUNT, 0, 0);

    for (int i = 0; i < count; ++i) {
        ListBoxItemData* pData = (ListBoxItemData*)SendMessageW(hListBox, LB_GETITEMDATA, i, 0);
        if (pData != nullptr) {
...
//释放所有结构体指针
void CleanupListBoxPtrs(HWND hListBox) {
    ...
    SendMessageW(hListBox, LB_RESETCONTENT, 0, 0); // 清空列表
}


// 函数：向 ListBox 添加一个项并绑定结构体
int AddListBoxItem(HWND hWnd, DWORD pid) {
    // 分配结构体
    ...
    // 构造显示字符串
    WCHAR buffer[256];
    swprintf_s(buffer, L"- PID: %u", pid);
    

    // 添加字符串到 ListBox
    int index = (int)SendMessageW(hListBox, LB_ADDSTRING, 0, (LPARAM)buffer);

    // 绑定结构体到该项
    SendMessageW(hListBox, LB_SETITEMDATA, index, (LPARAM)pData);

    return index; // 可返回 index 以便后续使用
}
    ...
    case WM_HOTKEY:
        if (wParam == HOTKEY_ID)
        {
            ...
            if (hForegroundWnd)
            {
                ...
                AddListBoxItem(hForegroundWnd, processId);
    ...
    case WM_COMMAND:
    {
        ...
        case ID_LISTBOX:
            if(HIWORD(wParam) == LBN_DBLCLK)
            {
                ...
                if (index != LB_ERR)
                {
                    LPARAM result = SendMessageW(hListBox, LB_GETITEMDATA, index, 0);
                    if (result != LB_ERR) {
                        ListBoxItemData* pData = (ListBoxItemData*)result;

                        if (pData != nullptr) {
                            if (_CrtIsValidHeapPointer(pData)) {
                                std::cout << "程序运行中..." << pData->pid << std::endl;
                                delete pData; 
                            }
                        }
                    }
            ...
            break;
        case IDM_ABOUT:{
            //std::string  bane= WoWGetPlayerName(40864);

            //SetWindowTextW(hStatusLabel, stringToWstring(bane).c_str());
...
void sendKeyToListBoxsDown(int key)
{
    ...
    if (key != '\0')
    {
        ...
        {
            ListBoxItemData* pData = (ListBoxItemData*)SendMessageW(hListBox, LB_GETITEMDATA, j, 0);

            if (pData != nullptr && pData->inCtrl)
            {
                HWND targetWindow =  pData->hWnd;
            ...
            case ID_LOGINS_OK: {
                ...
                if (!exeAddr.empty() && !acctAddr.empty()) {
                    ...
                    for (auto& proc : batchGameLogin::processList) {
                        AddListBoxItem(proc.hwnd, proc.pi.dwProcessId);
                    }
                }
                EndDialog(hDlg, LOWORD(wParam));
                return (INT_PTR)TRUE;
    ...
    try {
        if (remoteCtrlisTaskRunning) {
            ...
            case CommandParseEr::MessageType::COMMAND: {
                ...
                case CommandParseEr::CommandType::ACTIVE_PROCESS: {
                    std::wstring a =  data[L"CHECKBOX_ID"];
                    if (!is_all_digits(a)){
                        break; 
                    }
                        ...
                        for (int i = 0; i < count; ++i) {
                            ...
                            if (pData != nullptr) {
                                if (index == 999|| index == i) {
                                    pData->inCtrl = true; // 设置所有窗口为活动状态
                                }
                                
                                if(index == 998){
									pData->inCtrl = false; // 设置所有窗口不为活动状态
                                }
                            }
                        }
...
void closeListboxProcesses() {
    std::vector<ListBoxItemData*> items = GetAllListBoxItemData(hListBox);
    for (ListBoxItemData* data : items) {
        // 执行你要的逻辑，比如刷新状态、检测PID等
        HWND targetWindow = data->hWnd;
        if (IsWindow(targetWindow)) {
